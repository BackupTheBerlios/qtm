 ...
 * @since Mar 8, 2004
 * @version $Id: SizeGrip.java,v 1.1 2005/01/23 17:20:10 cowahl Exp $
 */
...
public final class SizeGrip extends Canvas
{
    ...
    addListener(SWT.MouseDown, new Listener()
    {
      public void handleEvent(Event event)
      {
        ...
        else if(event.button == 3 && (event.stateMask & SWT.BUTTON1) != 0) // chord click
        {
          ...
          {
            shell.setSize(snapBackX, snapBackY);
            snapBackX = 0;
            snapBackY = 0;
            cancelled = true;
          }
  ...
  private void performResize(Event event, Shell shell)
  {
    // Make sure we stay within the display's client area
    Rectangle dispCA = getDisplay().getClientArea();
    Point p2 = toControl(dispCA.x + dispCA.width - 1, dispCA.y + dispCA.height - 1);
    event.x = Math.min(event.x, p2.x);
    event.y = Math.min(event.y, p2.y);
    ...
    if(newX != p.x || newY != p.y) shell.setSize(newX, newY);
  }
